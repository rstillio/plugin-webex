...
func registerListMeetings(s *mcpserver.MCPServer, client *webex.Client) {
	tool := mcp.NewTool("list_meetings",
		mcp.WithDescription("List upcoming or recent Webex meetings. Defaults to showing the next 7 days of meetings. Use meeting_type=\"meeting\" to get instance IDs needed for transcript retrieval."),
		mcp.WithString("from",
		...
		mcp.WithString("meeting_type",
			mcp.Description("Type of meeting to list: \"meetingSeries\" (default) or \"meeting\" (individual instances, required for transcripts)."),
		),
		mcp.WithNumber("max",
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		meetings, err := client.ListMeetings(from, to, meetingType, max)
		if err != nil {
		...
		for _, m := range meetings {
			text += fmt.Sprintf("- **%s** (%s)\n  Time: %s → %s\n  State: %s | Host: %s\n  Meeting #: %s\n  ID: %s\n",
				m.Title, m.MeetingType, m.Start, m.End, m.State, m.HostDisplayName, m.MeetingNumber, m.ID)
			if m.Agenda != "" {
